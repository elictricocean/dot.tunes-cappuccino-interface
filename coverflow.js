...
function CVFL$(el)
{
	...
	{
		console.log("gone");
		el = _DOMElement.getElementById(el);
		console.log(el);
		/*if(el == "__cvfl-coverflow")
            el = coverFlowView;
        if(el == "__cvfl-coverflow-wrapper")
            el = coverFlowViewWrapper;
        if(el == "__cvfl-coverflow-label")
            el = coverFlowViewLabel;*/
    }
...
var Coverflow =
{
	...
	init: function(list, o, domelement)
	{
		...
		this.container = this.domelement.getElementById('__cvfl-coverflow');
		this.container.className = 'loading';
		this.wrapper = this.domelement.getElementById('__cvfl-coverflow-wrapper');
		this.wrapperAnim = new Anim(this.wrapper);
		this.label = this.domelement.getElementById('__cvfl-coverflow-label');
	...
	setup: function(list)
	{
		...
		this.domelement.getElementById('__cvfl-img-'+ this.selected).style.cursor = 'pointer';
	...
	center: function(n, dir, len, l, move)
	{
		...
		var step = function()
		{
			...
			else
			{
				...
				this.domelement.getElementById('__cvfl-img-'+ Coverflow.selected).style.cursor = 'pointer';
			}
		};
	...
	shiftIn: function(i, dir, old, sel)
	{
		if(i < old - 3 || i > old + 3)
		{
			...
			this.domelement.getElementById('__cvfl-img-'+ i).style.opacity = 0;
		}
		var d = Math.abs(old - sel);
		var el = CVFL$('__cvfl-img-'+ i);
		var l = parseInt(el.getStyle('left'));
